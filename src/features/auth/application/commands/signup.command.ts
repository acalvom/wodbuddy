...
export class SignupCommand implements Command<AuthUserRequest, AuthUser> {
	constructor(private readonly authRepository: AuthRepository) {}

	execute(authUserRequest: AuthUserRequest): Promise<AuthUser> {
		return this.authRepository.signup(authUserRequest.email, authUserRequest.password);
	}
}
